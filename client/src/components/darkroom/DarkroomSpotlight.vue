...
const emit = defineEmits(['next', 'viewDetail', 'selectPhoto'])
...
const updateContainerSize = () => {
  ...
  containerStyle.value = { width: maxW + 'px', height: maxH + 'px' }
}

const spawnRipple = (x, y) => {
  const color = currentColor.value.color
  const ripple = { id: Date.now() + Math.random(), x, y, size: 0, opacity: 0.7, color }
...
const triggerChargeEffect = () => {
  ...
  setTimeout(() => { chargeProgress.value = 0 }, 1500)
}
...
const handleMouseLeave = () => {
  if (stayTimer) { clearInterval(stayTimer); stayTimer = null }
  ...
  const fadeOut = setInterval(() => {
    ...
    if (stayDuration.value === 0) clearInterval(fadeOut)
  }, 50)
}
...
const toggleLight = () => {
  ...
  if (lightOn.value) stayDuration.value = 100
}
...
const viewDetail = () => { emit('viewDetail') }
...
<template>
  <div class="spotlight-wrapper">
    <div
      ...
      @dblclick="nextPhoto"
    >
      <img v-if="photo?.url" :src="photo.url" class="diary-photo" alt="diary" />
      <div class="noise-overlay"></div>
      <div class="diary-mask" :style="maskStyle"></div>

      <div v-if="chargeProgress > 0" class="charge-indicator"
        :style="{
      ...
      <div class="ripples">
        <div v-for="r in ripples" :key="r.id" class="ripple"
          :style="{
            left: r.x + '%', top: r.y + '%',
      ...
      <div class="trails">
        <div v-for="t in trails" :key="t.id" class="trail"
          :style="{
            left: t.x + '%', top: t.y + '%',
    ...
    <div class="controls">
      ...
      <button @click="prevPhoto" :disabled="currentIndex <= 0">上一张</button>
...
.ripples, .trails, .sparkles, .particles {
  position: absolute;
  inset: 0;
  pointer-events: none;
}
...
@keyframes sparkle-fly {
  ...
  20% { opacity: 1; }
  100% { transform: translate(var(--sparkle-end-x), var(--sparkle-end-y)) scale(0.2); opacity: 0; }
}
...
@keyframes particle-fly {
  ...
  100% { transform: translate(var(--particle-end-x), var(--particle-end-y)) scale(0.1); opacity: 0; }
}
